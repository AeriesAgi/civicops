...
1. Open `/Band`, launch the structural-fire scenario, and show five named agents coordinating through one Band room.
2. Open `/` and explain: “AI-powered civic reporting, routing and public alerts.” Residents report through mobile/PWA/web; Gemini/fallback structures reports; humans remain in control.
3. Open `/Home/Report`, submit a civic issue, and show the reference confirmation.
4. Open `/citizen-app` or `/app` to show the PWA citizen hub as the main public channel.
5. Open `/Home/Agent` and run: latest report, WhatsApp sandbox report, voice-note transcript, citizen response, department brief, alert recommendation and judge summary.
6. Open `/Home/Dashboard` to show queues and control-room cards.
7. Open `/Home/Lookup` and track a reference.
8. Open `/Home/Alerts` and `/Home/Weather` for public notices and area context.
9. Open `/Demo/WhatsAppSimulator` as optional connector readiness only.
10. Open `/Home/Connectors` and explain Gemini diagnostics, quota-safe cooldowns and WhatsApp environment gates.
